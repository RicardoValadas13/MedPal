...
export function AddMedicationPage() {
  ...
  async function handleSave() {
    if (!user || !medicationName || times.length === 0 || days.length === 0) return
    setSaving(true)
    setError(null)
    ...
    <div className="min-h-screen bg-[#f5f5f0] flex flex-col">
      ...
      <div className="flex-1 overflow-auto px-5 pb-32 space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm px-4 py-4">
          <label className="block text-xs font-semibold text-[#9ba0a3] uppercase tracking-wide mb-2">
        ...
        {/* Times card */}
        <div className="bg-white rounded-2xl shadow-sm px-4 py-4">
          <label className="block text-xs font-semibold text-[#9ba0a3] uppercase tracking-wide mb-3">
          ...
          {times.filter(t => !PRESET_TIMES.includes(t)).length > 0 && (
            <div className="flex flex-wrap gap-2 mb-3">
              {times.filter(t => !PRESET_TIMES.includes(t)).map(t => (
                <div
                  key={t}
                  className="flex items-center gap-1.5 h-9 pl-3 pr-2 bg-[#192830] text-white text-sm font-semibold rounded-xl"
                >
          ...
          {showCustomInput ? (
            <div className="flex items-center gap-2 mt-1">
              ...
              <button
                ...
                className="h-11 px-4 bg-[#192830] text-white text-sm font-semibold rounded-xl hover:opacity-90 transition"
              >
                Add
              </button>
              <button
                onClick={() => { setShowCustomInput(false); setCustomHour(''); setCustomMin('') }}
                className="h-11 w-11 flex items-center justify-center rounded-xl border border-[#e4e4de] text-[#9ba0a3] hover:text-[#43474a] transition"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <button
              onClick={() => { setShowCustomInput(true); setTimeout(() => hourRef.current?.focus(), 50) }}
        ...
        <div className="bg-white rounded-2xl shadow-sm px-4 py-4">
          <div className="flex items-center justify-between mb-3">
            <label className="block text-xs font-semibold text-[#9ba0a3] uppercase tracking-wide">
